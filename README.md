...
* a basecalled BAM file with reads from amplicon sequencing of TP53 (sample id: D25-test007_T),
* the BED files with the amplicon coordinates and primer coordinates,
* the input files `samples.tsv` and `units.tsv`.
...
2. Check the connection to the internet. The test must be run in an environment which can connect to the internet. 
This is necessary:
...
3. Decompress the reference genome if not done yet:
```bash
