...
The repository contains a setup for a small test in `.tests/integration` (structure before running the test):
...
* preprocessed files for skipping the steps that do not need to be tested:
  ...
  * the aligned BAM file located in `.tests/integration/reference/D25-test007_T_reads.ont_adapt_trim.filtered.aligned.bam`
  ...
  * VEP-annotated VCF files from ClairS-TO, DeepSomatic, VarDict and Sniffles v2 that are used to overwrite the VCF files
...
3. Check the connection to the internet. The test must be run in an environment which can connect to the internet. 
This is necessary:

 * to pull the Docker images into Singularity containers,
 * to use VEP with the option `--database`. Indeed, VEP will download on-the-fly the required but large cache files from Ensembl.

4. Decompress the reference genome if not done yet:
```bash
...
`.tests/integration/reference/D25-test007_T_reads.ont_adapt_trim.filtered.aligned.bam`

**Note about omitted rules**

 * the rules pertaining to basecalling with Dorado are not tested in this integration test for the following reasons:
 ...
 * the rule `pycoqc` is not tested in this integration test as it requires a sequencing summary file that is not provided here.
 ...
 * the rule `sequali` is not tested in this integration test as it requires the output files from `pycoqc`;
 * the rule `multiqc` is not tested in this integration test as it requires the output files from `pycoqc` and from `sequali`.
