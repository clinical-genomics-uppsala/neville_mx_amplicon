...
def get_gvcfs():
    return list(set([lambda wildcards: f"references/clairs_to/{wildcards.experiment}_{t.sample}_{t.type}/"
                                       f"{wildcards.experiment}_{t.sample}_{t.type}_{vartype}.g.vcf.gz"
                     for t in units.itertuples()
