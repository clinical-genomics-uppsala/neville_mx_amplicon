...
rule dorado_align:
    ...
    shell:
        ...
        if [ {params.runid} == "ABC123" ]; then
            ...
            samtools addreplacerg -r '@RG\tID:{wildcards.sample}\tSM:{wildcards.sample}' test_data/preprocessed/Mtest_D25-test007_T_reads.ont_adapt_trim.filtered.aligned.bam -o {output.bam} 2>> {log}
        fi
        """
...
rule aligning_samtools_calmd:
    ...
    output:
        bam=temp(
            "alignment/dorado_align/{experiment}_{sample}_{type}_reads.ont_adapt_trim.filtered.aligned.sorted.soft-clipped.nm.bam"
        ),
        bai=temp(
            "alignment/dorado_align/{experiment}_{sample}_{type}_reads.ont_adapt_trim.filtered.aligned.sorted.soft-clipped.nm.bam.bai"
        ),
    log:
